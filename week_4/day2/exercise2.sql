...
SELECT film_id, title, description, length, rental_rate FROM film WHERE title = 'Deep throat';
...
SELECT * FROM film ORDER BY replacement_cost LIMIT 10 OFFSET 10;
...
(SELECT film_id FROM inventory GROUP BY film_id);
...
country_id = (SELECT country_id FROM city WHERE city = 'Barcelona');
...
SELECT customer.customer_id, first_name, last_name, amount, payment_date FROM
